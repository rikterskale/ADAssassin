...
```powershell
$env:PIP_VERBOSE = "1"
python -m pip install . 2>&1 | Tee-Object -FilePath "$env:USERPROFILE\adassassin-install-logs\pip-install.log"
```

Contributor/test install (adds pytest/ruff and uses editable source):
...
python -m pip install . 2>&1 | tee "$HOME/adassassin-install-logs/pip-install.log"
```
