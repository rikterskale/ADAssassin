...
- every ADAssassin CLI option, runtime setting, and local API operation is
  documented in this guide, and the installed `adaf-attack` engine CLI remains
  discoverable through its complete built-in help;
...
server also reconciles jobs interrupted by an earlier shutdown. Every live
preflight is deliberately invalidated on restart because target reachability
and process-memory credentials cannot be assumed to survive. Open **Connect**,
re-enter any approved bind material, and run preflight again before YELLOW,
RED, or rollback-apply work.
...
| Navigation | Direct access to all 12 console surfaces |
...
7. Download the evidence bundle and retain its checksum manifest with the
   approved evidence set.
8. Read each closeout check.
...
If you make a mistake, select the engagement and use **Edit selected
engagement**. Name, target, and scope-note changes are audited. Changing the
domain or DC revokes the old preflight and clears in-memory credentials. Use
**Archive engagement** after retention handoff; archive is recoverable and
...
5. Complete every required prompt. Inputs are typed: constrained values use a
   selector, paths and identifiers use validated text fields, larger payloads
   use text areas, and sensitive prompts are masked without altering leading
...
Running jobs also appear in the global scope bar. If you leave Run, follow
that link to reattach to live polling. A local API interruption is displayed as
**status unknown** while automatic checks continue. Failed or
restart-interrupted jobs offer **Prepare a reviewed retry**; the product never
re-executes one automatically.
...
5. Download **Markdown**, **HTML**, and **evidence bundle**.
6. Open both reports and verify:
   ...
   - rollback leftovers;
   - closeout status.
7. Open `manifest.json` inside the ZIP and retain its SHA-256 inventory with
   the transferred evidence. The bundle includes portable engagement metadata,
   reports, and workspace evidence; it excludes plaintext bind credentials and
   reusable preflight state.
8. Store the exports in the approved evidence location.
9. Archive the engagement after handoff if the retention procedure calls for
   a read-only local record.
...
| Report | `/report` | Closeout checks, preview, MD/HTML, and checksummed evidence bundle |
...
| `ADASSASSIN_PREFLIGHT_TTL_SECONDS` | `900` | Maximum lifetime of a successful target preflight; minimum effective value is 30 seconds |
...
## 18. Discover every command and capability from the command line
...
ADAssassin installs the pinned ADAF-ATTACK engine as the `adaf-attack`
command. Its workflow helpers and direct engine surfaces are not hidden:

```bash
adaf-attack --help
adaf-attack guide
adaf-attack list-capabilities
adaf-attack capability-help CAPABILITY_ID
adaf-attack run --help
adaf-attack cleanup --help
adaf-attack support-bundle --help
```

Use `adaf-attack COMMAND --help` for every command displayed by the top-level
help. Direct engine execution is outside the GUI's state tracker, so use it
only when the operating procedure explicitly calls for it and preserve its
session evidence with the engagement.

## 19. Complete local API reference

The GUI uses the same local API documented here. It has 30 `/api` operations. There
...
| 3 | GET | `/api/guide?engagement_id={id}` | Guided milestones scoped to the selected engagement |
...
| 9 | PATCH | `/api/engagements/{id}` | Audited metadata edit; target changes revoke preflight |
| 10 | POST | `/api/engagements/{id}/archive` | JSON `archived`; archive or restore without deleting evidence |
...
| 18 | GET | `/api/engagements/{id}/findings/{finding_id}` | One finding |
| 19 | POST | `/api/engagements/{id}/findings/{finding_id}/explain` | Attach explanation/remediation |
| 20 | POST | `/api/engagements/{id}/findings/{finding_id}/status` | JSON status: open/accepted/fixed/retest |
| 21 | GET | `/api/engagements/{id}/vault` | Vault metadata inventory |
| 22 | POST | `/api/engagements/{id}/vault/{name}/unmask` | JSON scope and TTL 5–300 seconds |
| 23 | GET | `/api/engagements/{id}/rollback` | Rollback entries/sessions |
| 24 | POST | `/api/engagements/{id}/rollback/preview` | Offline cleanup preview |
| 25 | POST | `/api/engagements/{id}/rollback/apply` | Apply with ack, force, `YES`, optional session |
| 26 | GET | `/api/engagements/{id}/closeout` | Closeout readiness/checks |
| 27 | GET | `/api/engagements/{id}/report` | Generate JSON response and report content |
| 28 | GET | `/api/engagements/{id}/report.md` | Download Markdown report |
| 29 | GET | `/api/engagements/{id}/report.html` | Download HTML report |
| 30 | GET | `/api/engagements/{id}/bundle.zip` | Generate/download portable checksummed evidence bundle |
...
curl "$BASE_URL/api/guide?engagement_id=ENGAGEMENT_ID"
...
Correct metadata or archive/restore without deleting evidence:

```bash
curl -X PATCH "$BASE_URL/api/engagements/ENGAGEMENT_ID" \
  -H "Content-Type: application/json" \
  -d '{"name":"Corrected assessment name","domain":"corp.example","dc":"dc01.corp.example","notes":"Current authorization and scope."}'

curl -X POST "$BASE_URL/api/engagements/ENGAGEMENT_ID/archive" \
  -H "Content-Type: application/json" \
  -d '{"archived":true}'
...
curl -o engagement-evidence-bundle.zip "$BASE_URL/api/engagements/ENGAGEMENT_ID/bundle.zip"
```

### 19.11 Guided visit tracking

Only four page visits can be recorded manually; operational milestones are
always computed from the selected engagement's real state:
...
curl -X POST "$BASE_URL/api/engagements/ENGAGEMENT_ID/guided" \
  -H "Content-Type: application/json" \
  -d '{"step_id":"vault-review"}'
```

Doctor, workspace, connect, observe, rollback-preview, report, closeout, and
optional RED milestones complete only from real application state. They cannot
be marked done through the endpoint. RED never counts toward core completion.
